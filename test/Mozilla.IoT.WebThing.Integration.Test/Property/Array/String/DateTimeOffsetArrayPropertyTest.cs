...
{
    public class DateTimeOffsetArrayPropertyTest : AbstractArrayPropertyTest<DateTimeOffset>
    {
        protected override JsonElement CreateJson(IEnumerable<DateTimeOffset> values)
        {
        ...
        protected override List<DateTimeOffset> CreateValue(int arrayLength)
        {
            var values = new List<DateTimeOffset>(arrayLength);

            for (var i = 0; i < arrayLength; i++)
            {
                var value = Fixture.Create<DateTime>();
                values.Add(DateTime.Parse(value.ToString("U")));
            }
