...
{
    public class ParameterUIntTest
    {
        private readonly Fixture _fixture;
        
        public ParameterUIntTest()
        {
            _fixture = new Fixture();
        }
        
        #region No Nullable 
        private static ParameterUInt CreateNoNullable(uint[]? enums = null, uint? min = null, uint? max = null, uint? multipleOf = null)
            => new ParameterUInt(false, min, max, multipleOf, enums);
        ...
        private static ParameterUInt CreateNullable(uint[]? enums = null, uint? min = null, uint? max = null, uint? multipleOf = null)
            => new ParameterUInt(true, min, max, multipleOf, enums);
        ...
        {
            var value = Convert.ToUInt32(param);
            var property = CreateNullable(min: 10);
            var jsonElement = JsonSerializer.Deserialize<JsonElement>($@"{{ ""input"": {value} }}");
            property.TryGetValue(jsonElement.GetProperty("input"), out _).Should().BeFalse();
        }
