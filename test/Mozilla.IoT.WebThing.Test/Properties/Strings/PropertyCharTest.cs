...
namespace Mozilla.IoT.WebThing.Test.Properties.Strings
{
    public class PropertyCharTest
    {
        ...
        public void SetNoNullableWithValue()
        {
            ...
            var jsonElement = JsonSerializer.Deserialize<JsonElement>(JsonSerializer.Serialize(new Serializer{Input = value}, 
        ...
        public void TrySetNoNullableWithEnumValue()
        {
            var values = _fixture.Create<char[]>();
            var property = CreateNoNullable(values);
            var jsonElement = JsonSerializer.Deserialize<JsonElement>(JsonSerializer.Serialize(
                new Serializer{Input = _fixture.Create<char>()}, 
                new JsonSerializerOptions
                {
                    PropertyNamingPolicy = JsonNamingPolicy.CamelCase
                }));
