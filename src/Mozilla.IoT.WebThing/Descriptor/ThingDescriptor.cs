    ...
    public class ThingDescriptor : IDescriptor<Thing>
    {
        ...
        public IDictionary<string, object> CreateDescription(Thing value)
        {
            ...
            value.ActionsTypeInfo.ForEach(action =>
            {
                ...
                metadata.Add(LINKS,
                    new List<Dictionary<string, object>>(1)
                    {
                        new Dictionary<string, object>
                        {
            ...
            {
                [REL] = RelType.Properties.ToString().ToLower(), [HREF] = value.HrefPrefix.JoinUrl("properties")
            };

            var actionsLink = new Dictionary<string, object>
            {
                [REL] = RelType.Actions.ToString().ToLower(), [HREF] = value.HrefPrefix.JoinUrl("actions")
            };

            var eventsLink = new Dictionary<string, object>
            {
                [REL] = RelType.Events.ToString().ToLower(), [HREF] = value.HrefPrefix.JoinUrl("events")
            };

            var links = new List<IDictionary<string, object>> {propertiesLink, actionsLink, eventsLink};
